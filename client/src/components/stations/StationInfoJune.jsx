...
const StationInfoJune = () => {
  ...
  useEffect(() => {
    async function fetchData(id) {
      ...
      if (result.id) {
        setInfo(result);
        const result2 = await stationService.getJuneDepartures(id);
        setDepartures(result2);
        const result3 = await stationService.getJuneReturns(id);
        setReturns(result3);
        const result4 = await stationService.getJuneTopReturns(id);
        setTop5returns(result4);
        const result5 = await stationService.getJuneTopDepart(id);
        setTop5depart(result5);
        const result6 = await stationService.getJuneAvgDis(id);
        setAvgDisStart(result6);
        const result7 = await stationService.getJuneRetAvgDis(id);
